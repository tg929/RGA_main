...
PARSER.add_argument(
    ...
    Path may look like: /data2/ytg/autogrow4.0/mgltools_x86_64Linux2_1.5.6",
)
PARSER.add_argument(
    "--mgl_python",
    metavar="mgl_python",
    required=False,
    help="/data2/ytg/autogrow4.0/mgltools_x86_64Linux2_1.5.6/bin/pythonsh",
)
PARSER.add_argument(
    "--prepare_ligand4.py",
    metavar="prepare_ligand4.py",
    required=False,
    help="/data2/ytg/autogrow4.0/mgltools_x86_64Linux2_1.5.6/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_ligand4.py",
)
PARSER.add_argument(
    "--prepare_receptor4.py",
    metavar="prepare_receptor4.py",
    required=False,
    help="/data2/ytg/autogrow4.0/mgltools_x86_64Linux2_1.5.6/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_receptor4.py",
)
PARSER.add_argument(
    "--obabel_path",
...
PARSER.add_argument(
    ...
    default="/data2/ytg/autogrow4.0/autodock_vina_1_1_2_linux_x86/bin/vina",
...
#     return smiles_list 
def docking(smiles_folder, smiles_file, args_dict):
    # 第1步：将SMILES转换为SDF。这个函数会返回SDF文件被保存的路径。
    sdfs_folder_path = smiles_to_sdfs(args_dict, gen_smiles_file=os.path.join(smiles_folder, smiles_file), smile_file_directory=smiles_folder)

    # 第2步：将SDF转换为PDB。
    # 【关键修复】: `convert_sdf_to_pdbs` 将PDB输出到 `<gen_folder_path>_PDB/`，
    # 因此应将 `pdb_dir` 统一为 `<parent_of_sdf_dir>_PDB`，避免在错误目录 `PDBs/` 下查找。
    # PDB 将输出到 `<results_* >_PDB/`，其父目录应为 smiles_folder（每个 results_*）
    parent_of_sdf_dir = smiles_folder
    pdb_dir = parent_of_sdf_dir + "_PDB"
    conversion_to_3d.convert_sdf_to_pdbs(args_dict, gen_folder_path=parent_of_sdf_dir, sdfs_folder_path=sdfs_folder_path)

    # 第3步和第4步：继续进行PDB到PDBQT的转换和对接。
    docking_object = pdb_to_pdbqt(vars=args_dict, pdb_dir=pdb_dir)
    smiles_list = docking_pdbqt(args_dict, docking_object, pdb_dir, os.path.join(smiles_folder, smiles_file))
    return smiles_list
##########################################################
...
for receptor_info in receptor_info_list:
    ...
    new_smiles_list = initial_smiles_list[:] #### debug 
...
for num_gen in tqdm(range(args_dict['num_generations'])):
    for receptor_info in receptor_info_list:
        ...
        new_smiles_list = new_smiles_list[:] #### debug 
        if not os.path.exists(results_folder):
            os.makedirs(results_folder)
        full_smiles_file = os.path.join(results_folder, smiles_file)
        with open(full_smiles_file, 'w') as fout:
...
python RGA.py \
    ...
    --num_generations 10 \
    --mgltools_directory /data2/ytg/autogrow4.0/mgltools_x86_64Linux2_1.5.6/ \
